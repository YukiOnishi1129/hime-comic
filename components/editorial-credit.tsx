...
export function EditorialCredit({
  ...
  const description = (() => {
    switch (variant) {
      case "work":
        return "本作品ページの紹介文・要約・おすすめポイント・レビュー要約は、ひめコミ編集部がFANZAの公式情報および購入者レビューをもとに、AI（大規模言語モデル）を活用して整理・編集しています。価格・評価・ジャンルなどの事実情報はFANZAから直接取得しています。最終的な編集判断と公開責任はひめコミ編集部にあります。";
      case "feature":
        return "本特集ページの構成と紹介文は、ひめコミ編集部が販売実績・評価・サイトコンセプトを踏まえて作成・編集しています。AI（大規模言語モデル）を活用して整理しており、最終的な公開責任はひめコミ編集部にあります。";
      default:
        return "本ページのコンテンツは、ひめコミ編集部がFANZAの公式情報をもとに、AI（大規模言語モデル）を活用して整理・編集しています。最終的な編集判断と公開責任はひめコミ編集部にあります。";
    }
  })();
